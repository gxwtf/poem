...
/** 查询用户是否已收藏某句子 */
export const querySentenceStar = async (userId: number, poemId: string, text: string): Promise<boolean> => {
    const res = await prisma.sentenceStar.findUnique({
        where: {
            userId_poemId_text: { userId, poemId, text }
        }
    })
    return !!res
}
...
/** 切换句子收藏状态，返回收藏后的状态 */
export const toggleSentenceStar = async (userId: number, poemId: string, text: string): Promise<boolean> => {
